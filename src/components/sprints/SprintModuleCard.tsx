    ...
    >
      ...
      <div className="mb-4">
        <div className="flex items-start justify-between mb-2">
          ...
          {isLocked && (
            <div className="flex items-center justify-center w-8 h-8 bg-surface-300 dark:bg-surface-700 rounded-full">
        ...
        <h3 className="text-lg font-heading font-bold text-surface-900 dark:text-surface-100 mb-1">
          {module.title}
        </h3>
      ...
      {(isInProgress || isCompleted) && (
        <div className="mb-4 flex items-center gap-2">
          ...
          <div>
            <div className="text-lg font-heading font-bold text-accent-500 dark:text-accent-400">
              {xpEarned} XP
            </div>
            <div className="text-xs text-surface-500 dark:text-surface-500">earned in this module</div>
          </div>
        </div>
      )}
      ...
      {isLocked && (
        <div className="mb-4 p-3 bg-surface-200/50 dark:bg-surface-800/50 rounded-lg border border-surface-300 dark:border-surface-700">
          <div className="flex items-center gap-2 text-sm">
            <svg className="w-4 h-4 text-surface-500 dark:text-surface-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" />
            </svg>
            <span className="text-surface-600 dark:text-surface-400">
              Unlock at <span className="font-bold text-surface-800 dark:text-surface-300">{module.unlockThresholdXP} XP</span>
            </span>
          </div>
          {xpNeeded > 0 && (
            <div className="mt-1 text-xs text-surface-500 dark:text-surface-500">
              ({xpNeeded} XP to go)
            </div>
          )}
        </div>
      ...
      {!isLocked && module.concepts.length > 0 && (
        <div className="mt-4 pt-4 border-t border-surface-200 dark:border-surface-700">
          <div className="text-xs text-surface-500 dark:text-surface-500 mb-2">You'll practice:</div>
          <div className="flex flex-wrap gap-1">
            {module.concepts.slice(0, 4).map((concept, idx) => (
              <span
                key={idx}
                className="px-2 py-1 bg-surface-100 dark:bg-surface-800/50 text-surface-600 dark:text-surface-400 text-[10px] rounded"
              >
                {concept}
              </span>
            ))}
            {module.concepts.length > 4 && (
              <span className="px-2 py-1 text-surface-500 dark:text-surface-500 text-[10px]">
